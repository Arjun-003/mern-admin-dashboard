...
function CategoriesList() {
  ...
  const fetchCategories = useCallback(
    async (page, limit, search = "") => {
      try {
        ...
        const url = `${ApiEndPoint.categoryList}?${queryParams.toString()}`;
        ...
        if (response && response.data) {
          ...
          setCategories(response.data || []); // Accessing correct data structure
  ...
  useEffect(() => {
    fetchCategories(currentPage, entriesPerPage, debouncedSearchQuery);
  ...
  const handleDeleteCategory = async (id, title) => {
    Swal.fire({
      ...
      if (r.isConfirmed) {
        try {
          await deleteData(`${ApiEndPoint.deleteCategory}/${id}`);
          Swal.fire({
            icon: "success",
            title: "Deleted!",
            timer: 1500,
  ...
  const handleAddCategory = () => navigate("/category_add");
                    ...
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm font-medium space-x-3">
                        {/* <button
                          ...
                          onClick={() => navigate(`/category_view/${item.id}`)}
                        ...
                        <button
                          ...
                          onClick={() => navigate(`/category_edit/${item.id}`)}
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleDeleteCategory(item.id, item.title)
                          }
