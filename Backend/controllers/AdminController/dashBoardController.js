...
const dashBoard = {
  ...
  try {
    ...
    // 3️⃣ Total Sellers (distinct users in Products table)
...
sign_up: async (req, res) => {
        try {
            ...
            const userData = JSON.stringify({ name, email, mobile_Number, password, role });
...
verifyOtp: async (req, res) => {
        try {
            ...
            const { name, email, password, role } = JSON.parse(userDataJson);
